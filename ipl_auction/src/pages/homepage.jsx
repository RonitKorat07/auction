    ...
    useEffect(() => {
        ...
        gsap.utils.toArray(".player-card").forEach((card, index) => {
            gsap.fromTo(
                card,
                { opacity: 0, y: 100 },
                { opacity: 1, y: 0, duration: 0.8, delay: index * 0.2, scrollTrigger: { trigger: card, start: "top bottom-=100", end: "bottom center", toggleActions: "play none none reverse", scrub: 1 } }
            );
        });
        ...
        <>
            ...
            <div className="flex min-h-screen w-full flex-col lg:flex-row items-center justify-center gap-6 bg-[url('https://i.pinimg.com/736x/7d/57/bb/7d57bb24b1d299219c693bdf8600946d.jpg')] bg-no-repeat bg-cover bg-center text-white">
                <div className="flex h-auto w-full flex-col lg:flex-row items-center justify-center bg-[rgba(0,0,0,0.3)] gap-6 px-4 py-8 text-white">
                    <div ref={bgRef} className="relative w-full h-[550px] sm:w-[80%] md:w-[70%] lg:w-1/2 sm:h-[550px] md:h-[630px] lg:h-[650px] xl:h-[700px] flex items-center justify-center pt-10">
                        <motion.img
                    ...
                    <div className="w-full text-center lg:w-1/2 px-4">
                        <motion.p ref={adminTagRef} className="text-2xl font-bold text-white text-opacity-80 uppercase tracking-wide pb-10">
                        ...
                        <motion.div className="mt-10 flex justify-center">
                            <Link to={"/auction"} className="start-auction-btn group relative px-8 py-4 bg-gradient-to-r from-purple-500 to-blue-600 rounded-xl text-xl font-bold text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden">
              ...
              >
                ...
                <div className="max-w-7xl mx-auto px-4 py-12">
                            ...
                            topBatsmen.map((player,index) => (
                              ...
                              >
                                    ...
                                    <p className="text-sm text-blue-300 font-bold mt-2">
                                        Matches: {player.state?.ipl?.batting?.match || 0} | Runs: {player.state?.ipl?.batting?.runs || 0} | Avg: {player.state?.ipl?.batting?.average || 0}
                                    </p>
                                </motion.div>
                            ))
                        )}
                    ...
                    <motion.div
                        ...
                        className="flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-0 mb-8 sm:mb-12 mt-12"
                    >
                        <h1 className="text-4xl font-bold text-blue-400">Top 3 All-Rounders</h1>
                        <button
                            onClick={() => handleNavigate("/players")}
                            className="flex items-center justify-center bg-blue-500 text-white px-4 py-2 rounded-full hover:bg-blue-700 transition"
                        >
                        ...
                        {playersLoading ? (
                            ...
                            topAllRounders.map((player,index) => (
                              ...
                              >
                                    ...
                                    <p className="text-sm text-blue-300 font-bold mt-2">
                                        Matches: {player.state?.ipl?.batting?.match || 0} | Runs: {player.state?.ipl?.batting?.runs || 0} | Wickets: {player.state?.ipl?.bowling?.wicket || 0}
                                    </p>
                                </motion.div>
                            ))
                        )}
